    ...
    private fun connectToSignalingServer() {
        ...
        ws = object : WebSocketClient(URI(wsUrl)) {
            ...
            override fun onClose(code: Int, reason: String?, remote: Boolean) {
                runOnUiThread {
                    Toast.makeText(this@MainActivity, "信令服务器连接已关闭", Toast.LENGTH_SHORT).show()
                }
            }
            
            override fun onError(ex: Exception?) {
                runOnUiThread {
                    Toast.makeText(this@MainActivity, "信令服务器连接失败: ${ex?.message}", Toast.LENGTH_SHORT).show()
                    connectButton.text = "开始投屏"
                    isConnected = false
                }
            }
    ...
    private fun setupWebRTCConnection(data: Intent) {
        ...
        videoCapturer = ScreenCapturerAndroid(
            ...
            object : MediaProjection.Callback() {
                override fun onStop() {
                    runOnUiThread {
                        Toast.makeText(this@MainActivity, "屏幕捕获已停止", Toast.LENGTH_SHORT).show()
                    }
                }
            }
        )
            ...
            override fun onIceConnectionChange(state: PeerConnection.IceConnectionState?) {
                when (state) {
                    ...
                    PeerConnection.IceConnectionState.CLOSED -> {
                        runOnUiThread {
                            Toast.makeText(this@MainActivity, "连接状态: $state", Toast.LENGTH_SHORT).show()
                        }
                    }
                    else -> {}
                }
        ...
        peerConnection!!.createOffer(object : SdpObserver {
            override fun onCreateSuccess(sessionDescription: SessionDescription) {
                ...
                peerConnection!!.setLocalDescription(object : SdpObserver {
                    ...
                    override fun onSetFailure(p0: String?) {
                        runOnUiThread {
                            Toast.makeText(this@MainActivity, "设置本地描述失败", Toast.LENGTH_SHORT).show()
                        }
                    }
                }, sessionDescription)
            }
            ...
            override fun onCreateFailure(error: String?) {
                runOnUiThread {
                    Toast.makeText(this@MainActivity, "创建offer失败: $error", Toast.LENGTH_SHORT).show()
                }
            }
